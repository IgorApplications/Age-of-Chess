...
public class MainView implements Screen {
    ...
    private void initButtons() {
        ...
        buttonPlay.addListener(new ChangeListener() {
            ...
            public void changed(ChangeEvent event, Actor actor) {
                if (buttonGroup.getChecked() != null) {
                    Settings.gdxGame.goToGame(MainView.this, stage);
                }
            }
        });
        ...
        for (Level level : Settings.account.availableLevels()) {
            TextButton levelButton = new TextButton(Settings.controller.defineLevelText(level), Settings.gdxGame.getButtonSkin(), "toggle");
            levelButton.getLabel().setFontScale(0.5f);

            levelButton.row().colspan(3).padRight(16).padBottom(3);
            levelButton.add(Settings.controller.getRecordResImage(level, 100)).size(12, 12).align(Align.bottomRight);
            levelButton.add(Settings.controller.getRecordResImage(level, 50)).size(12, 12).align(Align.bottomRight);
            levelButton.add(Settings.controller.getRecordResImage(level, 25)).size(12, 12).align(Align.bottomRight);
            levelButton.row();
